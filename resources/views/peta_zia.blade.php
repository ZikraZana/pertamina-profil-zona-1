...
<head>
    ...
    <style>
        ...
        /* Provinsi yang wilayah kerjanya sedang aktif/dipilih -> warna hijau */
        .prov.prov-selected {
        ...
        .floating-card .detail-btn:hover {
            background: #2547a8;
        }
...
<body class="bg-slate-100 h-screen flex flex-col p-6 overflow-hidden">
    ...
    <h1 class="text-2xl font-bold text-blue-900 mb-4 shrink-0">Peta Wilayah Kerja — Pertamina Hulu Rokan Regional 1 Zona
        1</h1>
    ...
    <div class="relative flex-1 bg-white rounded-xl shadow-lg border border-slate-300 p-4 min-h-0">
        <svg class="w-full h-full" viewBox="0 0 460 413" preserveAspectRatio="xMidYMid meet" role="img">
            ...
            @foreach ($wilayahKerja as $w)
                <line class="wk-line" data-line="{{ $w['kode'] }}" x1="{{ $w['dx'] }}" y1="{{ $w['dy'] }}"
                    x2="{{ $w['bx'] }}" y2="{{ $w['by'] }}" />
            @endforeach
            ...
            @foreach ($wilayahKerja as $w)
                <foreignObject class="wk-badge" data-badge="{{ $w['kode'] }}" x="{{ $w['bx'] - 48 }}"
                    y="{{ $w['by'] - 11 }}" width="96" height="24" style="overflow: visible;">
                    <div xmlns="http://www.w3.org/1999/xhtml" style="text-align:center;">
                        <span class="wk-badge-inner" data-badge-click="{{ $w['kode'] }}"
                            data-nama-click="{{ $w['nama'] }}" data-provinsi-click="{{ $w['provinsi'] }}">
                            {{ $w['nama'] }}
                        </span>
                    </div>
                </foreignObject>
    ...
    <div id="modal-overlay" class="modal-overlay">
        <div class="modal-content" id="modal-content">
            ...
            <h2 id="modalTitle" class="text-lg font-bold text-blue-900 mb-4 pr-6">-</h2>
            <div id="modalBody" class="text-sm space-y-2 text-slate-700"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function openCards() {
            ...
            document.getElementById('card-fasilitas').classList.add('show');
        }

        function closeCards() {
            document.getElementById('card-info').classList.remove('show');
            document.getElementById('card-produksi').classList.remove('show');
            document.getElementById('card-fasilitas').classList.remove('show');
        }

        let activeProvinsi = null; // kode provinsi yang path-nya sedang disorot hijau

        function selectProvinsi(provinsiCode) {
            if (activeProvinsi) {
                document.querySelector(`.prov[data-p="${activeProvinsi}"]`)?.classList.remove('prov-selected');
            }
            activeProvinsi = provinsiCode;
            document.querySelector(`.prov[data-p="${activeProvinsi}"]`)?.classList.add('prov-selected');
        }
        ...
        function setActive(key, nama, provinsiCode) {
        ...
        function loadWilayahKerja(kode, namaWilayah) {
            ...
            // ============================================================
            // MODE TAMPILAN SAJA (belum ada API/backend):
            // data di bawah ini statis/dummy, hanya untuk keperluan membangun
            // & menguji tampilan. Nanti kalau API sudah siap, tinggal ganti
      ...
      `;
        ...
        document.addEventListener('click', (e) => {
            const clickedInsideCard = e.target.closest('.floating-card');
            const clickedInsideStaticCard = e.target.closest('.static-card');
