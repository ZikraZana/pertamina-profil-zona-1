...
<head>
    <meta charset="UTF-8">
    <title>Peta Wilayah Kerja Sumatra — Regional 1</title>
    ...
    <style>
        ...
        /* Nama provinsi, statis, selalu tampil di area masing-masing */
        .prov-name {
            font-size: 9px;
            font-weight: 600;
            fill: #ffffff;
        ...
        /* Garis penghubung dari titik ke badge */
        .wk-line {
            stroke: #1e293b;
            stroke-width: 1.2;
            opacity: 0;
        ...
        /* Badge label wilayah kerja (di dalam foreignObject) */
        .wk-badge {
            opacity: 0;
            transition: opacity .15s ease;
            pointer-events: none;
        ...
        .wk-badge-inner:hover {
            background: #2547a8;
        }

        /* Card floating di sebelah kanan peta */
        .floating-card {
            position: absolute;
            width: 260px;
            background: #ffffff;
...
<body class="bg-slate-100 h-screen flex flex-col p-6 overflow-hidden">

    <h1 class="text-2xl font-bold text-blue-900 mb-4 shrink-0">Peta Wilayah Kerja — Pertamina Regional 1 (Sumatra)</h1>

    {{-- Wrapper peta: mengisi penuh sisa ruang (lebar & tinggi) --}}
    <div class="relative flex-1 bg-white rounded-xl shadow-lg border border-slate-300 p-4 min-h-0">
        <svg class="w-full h-full" viewBox="0 0 460 413" preserveAspectRatio="xMidYMid meet" role="img">
                ...
                <path class="prov" fill="#003399" data-p="ID-RI" data-nama="Riau"
                ...
                <path class="prov" fill="#003399" data-p="ID-SU" data-nama="Sumatera Utara"
                ...
                <path class="prov" fill="#003399" data-p="ID-SB" data-nama="Sumatera Barat"
                ...
                <path class="prov" fill="#003399" data-p="ID-SS" data-nama="Sumatera Selatan"
                ...
                <path class="prov" fill="#003399" data-p="ID-JA" data-nama="Jambi"
                ...
                <path class="prov" fill="#003399" data-p="ID-LA" data-nama="Lampung"
                ...
                <path class="prov" fill="#003399" data-p="ID-BE" data-nama="Bengkulu"
                ...
                <path class="prov" fill="#003399" data-p="ID-AC" data-nama="Aceh"
                ...
                {{-- ============================================================
                     ...
                     ============================================================ --}}
                <text class="prov-name" x="75" y="60">Aceh</text>
                <text class="prov-name" x="150" y="128">Sumatera Utara</text>
                <text class="prov-name" x="191" y="222">Sumatera Barat</text>
                <text class="prov-name" x="230" y="195">Riau</text>
                <text class="prov-name" x="255" y="255">Jambi</text>
                <text class="prov-name" x="255" y="327">Bengkulu</text>
                <text class="prov-name" x="310" y="305">Sumatera Selatan</text>
                <text class="prov-name" x="337" y="360">Lampung</text>

                {{-- ============================================================
                     6 WILAYAH KERJA — titik + garis + badge (muncul saat hover)
                     ============================================================ --}}
                @php
                    // dx,dy = posisi titik wilayah kerja di peta (dalam koordinat viewBox 460x413)
                    // bx,by = posisi badge label ditarik ke arah mana
                    // kode  = dipakai untuk memanggil /api/wilayah-kerja/{kode} — sesuaikan
                    //         dengan kode asli di database/API Anda.
                    $wilayahKerja = [
                        ['kode' => 'nso', 'nama' => 'PHE-NSO', 'dx' => 97, 'dy' => 43, 'bx' => 150, 'by' => 10],
                        ['kode' => 'rantau', 'nama' => 'Rantau', 'dx' => 110, 'dy' => 73, 'bx' => 158, 'by' => 38],
                        [
                            'kode' => 'p-susu',
                            'nama' => 'Pangkalan Susu',
                            'dx' => 125,
                        ...
                        ['kode' => 'lirik', 'nama' => 'Lirik', 'dx' => 250, 'dy' => 210, 'bx' => 335, 'by' => 165],
                        ['kode' => 'jambi', 'nama' => 'Jambi', 'dx' => 282, 'dy' => 250, 'bx' => 355, 'by' => 210],
                        [
                            'kode' => 'jambi-merang',
                            'nama' => 'Jambi Merang',
                            'dx' => 299,
                ...
                @foreach ($wilayahKerja as $w)
                    <line class="wk-line" data-line="{{ $w['kode'] }}"
                        x1="{{ $w['dx'] }}" y1="{{ $w['dy'] }}"
                        x2="{{ $w['bx'] }}" y2="{{ $w['by'] }}" />
                @endforeach

                @foreach ($wilayahKerja as $w)
                    <circle class="wk-dot" data-dot="{{ $w['kode'] }}"
                        cx="{{ $w['dx'] }}" cy="{{ $w['dy'] }}" r="2.8" />
                @endforeach

                @foreach ($wilayahKerja as $w)
                    <foreignObject class="wk-badge" data-badge="{{ $w['kode'] }}"
                        x="{{ $w['bx'] - 48 }}" y="{{ $w['by'] - 11 }}" width="96" height="24"
                        style="overflow: visible;">
                        <div xmlns="http://www.w3.org/1999/xhtml" style="text-align:center;">
                            <span class="wk-badge-inner" data-badge-click="{{ $w['kode'] }}" data-nama-click="{{ $w['nama'] }}">
                                {{ $w['nama'] }}
                            </span>
                        </div>
                    </foreignObject>
            ...
            {{-- CARD 1: Informasi umum wilayah kerja (posisi kanan-atas, floating) --}}
            ...
            {{-- CARD 2: Produksi & fasilitas (posisi kanan-bawah, floating) --}}
            <div id="card-produksi" class="floating-card" style="top: 260px; right: 24px;">
                <button class="close-btn" id="closeCardProduksi">&times;</button>
                <h2 class="text-base font-bold text-blue-900 mb-2 pr-4">Produksi &amp; Fasilitas</h2>
    ...
    <script>
        ...
        function showBadge(key) {
            ...
            document.querySelector(`.wk-badge[data-badge="${key}"]`)?.classList.add('show');
            document.querySelector(`.wk-line[data-line="${key}"]`)?.classList.add('show');
        }

        function hideBadgeNow(key) {
            clearTimeout(hideTimers[key]);
            document.querySelector(`.wk-badge[data-badge="${key}"]`)?.classList.remove('show');
            document.querySelector(`.wk-line[data-line="${key}"]`)?.classList.remove('show');
        }

        function scheduleHide(key) {
            if (key === activeKey) return; // jangan hilang kalau panelnya sedang terbuka untuk wilayah kerja ini
            clearTimeout(hideTimers[key]);
            hideTimers[key] = setTimeout(() => hideBadgeNow(key), HIDE_DELAY);
        }

        function openCards() {
            document.getElementById('card-info').classList.add('show');
            document.getElementById('card-produksi').classList.add('show');
        }

        function closeCards() {
            document.getElementById('card-info').classList.remove('show');
            document.getElementById('card-produksi').classList.remove('show');
        }

        function setActive(key, nama) {
        ...
        // Karena dot punya pointer-events:none (supaya tidak menghalangi klik provinsi
        // di bawahnya), area hover dibuat lewat sebuah lingkaran transparan yang lebih
        // besar, ditambahkan lewat JS.
        ...
        document.querySelectorAll('.wk-badge').forEach(fo => {
            const key = fo.dataset.badge;
            fo.addEventListener('mouseenter', () => showBadge(key));
            fo.addEventListener('mouseleave', () => scheduleHide(key));
        });
        document.querySelectorAll('[data-badge-click]').forEach(span => {
            span.addEventListener('click', (e) => {
                e.stopPropagation();
                setActive(span.dataset.badgeClick, span.dataset.namaClick);
            });
        });

        document.getElementById('closeCardInfo').addEventListener('click', closePanel);
        document.getElementById('closeCardProduksi').addEventListener('click', closePanel);

        function loadWilayahKerja(kode, namaWilayah) {
            ...
            infoBody.innerHTML = '<p class="text-slate-400">Memuat data...</p>';
            produksiBody.innerHTML = '<p class="text-slate-400">Memuat data...</p>';

            fetch(`/api/wilayah-kerja/${kode}`)
                .then(res => res.json())
                .then(res => {
                    if (!res.found) {
                        infoBody.innerHTML = `<p class="text-slate-400 italic">${res.message}</p>`;
      ...
      `;
                ...
                .catch(() => {
                    infoBody.innerHTML = '<p class="text-red-400 italic">Gagal memuat data. Cek koneksi API.</p>';
                    produksiBody.innerHTML = '';
                });
        }
    </script>
